...
(defconst spacegray-description
  "A Light Table inspired color theme based on Solarized's definitions.")

(defcustom spacegray-degrade nil
  "For test purposes only; when in GUI mode, forces spacegray to use the 256
degraded color mode to test the approximate color values for accuracy."
  :type 'boolean
  :group 'lt)

(defcustom spacegray-diff-mode 'normal
  "Sets the level of highlighting to use in diff-like modes."
  :type 'symbol
  :options '(high normal low)
  :group 'lt)

(defcustom spacegray-bold t
  "Stops spacegray from displaying bold when nil."
  :type 'boolean
  :group 'lt)

(defcustom spacegray-underline t
  "Stops spacegray from displaying underlines when nil."
  :type 'boolean
  :group 'lt)

(defcustom spacegray-italic t
  "Stops spacegray from displaying italics when nil."
  :type 'boolean
  :group 'lt)

(defcustom spacegray-contrast 'normal
  "Stick with normal! It's been carefully tested. Setting this option to high or
low does use the same spacegray palette but simply shifts some values up or
down in order to expand or compress the tonal range displayed."
  :type 'symbol
  :options '(high normal low)
  :group 'lt)

(defcustom spacegray-broken-srgb (and (eq system-type 'darwin)
                                     (eq window-system 'ns)
                                     (not (when (boundp 'ns-use-srgb-colorspace)
                                            ns-use-srgb-colorspace)))
  "Emacs bug #8402 results in incorrect color handling on Macs. If this is t
\(the default on Macs), spacegray works around it with alternative colors.
However, these colors are not totally portable, so you may be able to edit
the \"Gen RGB\" column in spacegray-definitions.el to improve them further."
  :type 'boolean
  :group 'lt)
...
(defvar spacegray-colors           ; ANSI(spacegray terminal)
  ...
  "This is a table of all the colors used by the spacegray color theme. Each
...
(defun spacegray-color-definitions (mode)
  (noct-flet ((find-color (name)
           (let* ((index (if window-system
                             (if spacegray-degrade
                                 3
                               (if spacegray-broken-srgb 2 1))
                           (case (display-color-cells)
                             (16 4)
                             (8  5)
                             (otherwise 3)))))
             (nth index (assoc name spacegray-colors)))))
    (let ((base03      (find-color 'base03))
          ...
          (bold        (if spacegray-bold 'bold 'normal))
          (bright-bold (if spacegray-bold 'normal 'bold))
          (underline   (if spacegray-underline t nil))
          (opt-under   nil)
          (italic      (if spacegray-italic 'italic 'normal)))
      (let ((back base03))
        (cond ((< (display-color-cells) 16)
               (setf back nil))
              ((eq 'high spacegray-contrast)
               (let ((orig-base3 base3))
                 (rotatef base01 base00 base0 base1 base2 base3)
                 (setf base3 orig-base3)))
              ((eq 'low spacegray-contrast)
               (setf back      base02
                     opt-under t)))
       ...
       (git-gutter:deleted ((t (,@fg-red))))
             ...
             (erc-nick-defauspacegray-face ((t (,@fmt-none ,@fg-cyan))))
...
(defmacro create-spacegray-theme ()
  (let* ((theme-name 'spacegray)
         (defs (spacegray-color-definitions 'dark))
         (theme-vars (mapcar (lambda (def) (list (car def) (cdr def)))
                             (second defs)))
         (theme-faces (first defs)))
    `(progn
       (deftheme ,theme-name ,spacegray-description)
